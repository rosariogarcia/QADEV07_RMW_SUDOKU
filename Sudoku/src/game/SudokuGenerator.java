...
public class SudokuGenerator {
        ...
        for (int i = 0; i < 9; i++)
            for (int j = 0; j < 9; j++) {
                ...
                if (Math.random() <= holeChance) {
                    ...
                    filledPositions.add(new Position(i,j));
                remainingSquares--;
            }
    }
    ...
    /*public static void main(String[] args) {
        ...
        sg.nextBoard(35);
        sg.print();
        System.out.println(sg.getSolver() != null);
    }*/

    public int[][] getSolver() {
        
        //int[][] cloneBoard = Arrays.copyOf(this.board, this.board.length);
        int[][] cloneBoard = ValidateSudoku.twoDimensionalArrayClone(board);
        //System.arraycopy(this.board, 0, cloneBoard, 0, 9);
        Backtracking a = new Backtracking();
        
        return a.solver(cloneBoard.clone());
    }
    public int[][] getBoard() {
        return board;
    }
    ...
    public void printTxt(String path){
        ...
        write.writeFileArray(board, path);    
    }      
